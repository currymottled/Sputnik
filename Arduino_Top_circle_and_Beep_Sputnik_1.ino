...
int C = 3;
int D = 4;
int Delay =500; //Sets the variable for the delay function so it can be adjusted easier.
...
int buzzerPitch = 1000; //value of the pitch for the tone function during deincrement beeps
...
void loop() //Cycles through the desired sequence of the above sequences of pin definitions and HIGH/LOW settings
{

  {
    ...
    setLED(6);
    delay(Delay);
    //Values of other LED's commented out but no delted due to possible future use by inviduals adapting the code
    //setLED(7);
    //delay(Delay);
    //setLED(8);
    //delay(Delay);
...
tone(buzzerPin, buzzerPitch ); //Turn on buzzer for 500 miliseconds then waits for 6000 to initiate cycle again
  delay(500);
  noTone(buzzerPin);
  delay(6000);
